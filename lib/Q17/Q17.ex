defmodule Q17 do
  ...
  def parse_target_area(text) do
    [x_min, x_max, y_min, y_max] =
      Regex.run(~r/target area: x=(\d+)\.\.(\d+), y=([-\d]+)\.\.([-\d]+)/, text)
      |> tl
  ...
  def simulation(
        init_state,
        %{x_min: x_min, x_max: x_max, y_min: y_min, y_max: y_max} = _target_area,
        n
      ) do
    0..(n - 1)
    |> Enum.reduce_while(
      ...
      fn _t, [{state, _} | _] = acc ->
        ...
        # I can't reach target area anymore or I already reached
        if y < y_min || is_target_area do
          {:halt, new_acc}
        else
          {:cont, new_acc}
        end
  ...
  def random_guess(
        %{x_min: x_min, x_max: _x_max, y_min: y_min, y_max: _y_max} = target_area,
        n_sim,
        n_trials
      ) do
    maxX = x_min
