defmodule Q23 do
  ...
  def read_and_parse(filename) do
    ...
    |> Map.new
    |> Map.filter(fn {_k, v} -> v != "" end) # Remove empty spaces
  end
  ...
  def bad_amphs(state, i_max) do
    ...
    ready_rooms = for amp <- Map.keys(rooms), into: %{} do
      {amp, is_room_ready(amp, state)}
    end

    good_amphs_score = ready_rooms
  ...
  def get_moves(state) do
    ...
    rooms_states = for amp <- Map.keys(rooms), into: %{} do
  ...
  def astar_search(start, goal, h) do
    open_set0 = Prioqueue.new([{0,start}], cmp_fun: &__MODULE__.cmp_fun/2)   # set of opened points
    came_from0 = %{} # where a node came from

    gScore0 = %{start =>  0} # effective cost from start to node n

    {came_from, gScore} = Stream.cycle([1])
    |> Enum.reduce_while(%{open_set: open_set0, gScore: gScore0, came_from: came_from0},
      fn _, %{open_set: open_set, gScore: gScore, came_from: came_from} ->
        case Prioqueue.extract_min(open_set)  do
          {:error, :empty} ->
            IO.puts("Queue is empty!!!")
            {:halt, {came_from , gScore}} # error
          {:ok, {{_, ^goal}, _}} ->
            {:halt, {came_from, gScore}} # goal reached! end loop, return path and costs
          {:ok, {{_est_cost, current_node}, open_set}} ->
            new_acc = get_moves(current_node)
            |> Enum.reduce(%{open_set: open_set, gScore: gScore, came_from: came_from},
              fn {n, cost}, %{open_set: open_set, gScore: gScore, came_from: came_from} = acc ->
                ...
                if tentative_gScore < gScore[n] do
                  ...
                  open_set = if !Prioqueue.member?(open_set, {nil, n}) do
  ...
  def part_ii(file \\ "lib/Q15/data2") do
    ...
    {path, costs} = astar_search(start_point, goal, h)
    IO.puts(path |> length())
    Enum.map(path, fn state -> print_state(state); IO.puts("\n") end)
    IO.puts("# Nodes open #{map_size(costs)}")
    IO.puts("Cost is #{costs[goal]}")
  end
