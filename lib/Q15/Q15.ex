defmodule Q15 do
  ...
  def astar_search(start, goal, environment, h) do
    open_set = Prioqueue.new([{0,start}])   # set of opened points
    came_from = %{} # where a node came from

    gScore = %{start =>  0} # effective cost from start to node n
    fScore = %{start => h.(start)} # estimated cost from start to node n: gScore + fScore at node n

    came_from = Stream.cycle([1])
    |> Enum.reduce_while(%{open_set: open_set, gScore: gScore, fScore: fScore, came_from: came_from},
      fn _, %{open_set: open_set, gScore: gScore, fScore: fScore, came_from: came_from} ->
        case Prioqueue.extract_min(open_set)  do
          {:error, :empty} -> {:halt, []} # error
          {:ok, {{_, ^goal}, _}} -> {:halt, came_from} # i reach goal! end loop and return path

          {:ok, {{_, current_node}, open_set}} ->
            new_acc = get_neighbors(current_node, environment)
            |> Enum.reduce(%{open_set: open_set, gScore: gScore, fScore: fScore, came_from: came_from},
              fn {n, z}, %{open_set: open_set, gScore: gScore, fScore: fScore, came_from: came_from} = acc ->
                ...
                if tentative_gScore < gScore[n] do
                  ...
                  fScore    = Map.put(fScore, n, tentative_gScore + h.(n)) #|> IO.inspect()

                  open_set = if !Prioqueue.member?(open_set, n) do
  ...
  def expand_map(%{x_max: x_max, y_max: y_max, points: points}, factor) do
    ...
    |> Enum.flat_map(
      fn {f,g} ->
        ...
        |> Enum.reduce([],
          fn {{x,y}, z}, acc ->
            [{{f*(x_max+1) + x, g*(y_max+1) + y}, Integer.mod(z+f+g-1, 9)+1 } | acc]
          end
      )
      end
    )
