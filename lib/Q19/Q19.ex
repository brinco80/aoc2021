...
#    rotation |> IO.inspect()
  ...
  def transformation_ik(%{offset: offset_ij, rotation: rotation_ij}, %{offset: offset_jk, rotation: rotation_jk}) do
  ...
  def inv_offset(offset_fwd, rotation_inv) do
    minus_offset  = 0..(tuple_size(offset_fwd)-1)
  ...
  def compute_rotation(dp, dq) do
    ...
    |> Enum.reverse()
    |> Enum.with_index
    |> Enum.map(fn {{i,s},k} -> {i, {k,s}} end) |> Map.new
  end
  ...
  def transformations_chain({i, j} = key, transformations) do
    case transformations do
      ...
      _ ->
        ...
        available_trx
        |> Enum.filter(fn {_i, k} ->  k == j end ) # candidates to match missing links
        |> Enum.reduce_while([ ],
          fn {k, _j}, _acc ->
            test_key = {k,j}
            acc = [ test_key ] # add transformation to chain

            # search for trx {i,k}
            parent_key = {i,k}
            case transformations do
              %{^parent_key => _trx} -> # found last link: done!
                {:halt, [parent_key | acc]}
              _ ->
                ...
                case transformations_chain({i,k}, new_transformations) do
                  [] ->
                    {:cont, []} # no luck! continue ¬¬
                  chain ->
                    {:halt, chain ++ acc} # sucess! concat and stop
                end
            end
          end
        )
      ...
      fn i ->
        ...
        case transformations do
          %{^key => x} ->
            {key, x}
          _ ->
            [last | rest] = transformations_chain(key, transformations)
            |> Enum.reverse()

            last_trx = transformations[last]
            trx = rest
            |> Enum.reduce(last_trx,
              fn t, acc ->
                current_trans = transformations[t]
                transformation_ik(current_trans,acc) end
            )
      ...
      fn i ->
        ...
        |> Enum.flat_map(
          fn j ->
            ...
            beacons_equivalence_map = find_similar_points_by_distance(distances[i],distances[j])
            |> equivalent_beacons
            |> Map.new

            if map_size(beacons_equivalence_map) >= n_overlap do
              IO.puts("Scanner #{i} overlaps with #{j} (#{map_size(beacons_equivalence_map)} beacons)")


              {p0, q0, dp, dq} = beacons_equivalence_map
              |> Map.to_list()
              ...
              [
                {
                  {i,j},
                  %{offset: offset_fwd, rotation: rotation_fwd} # fwd transformation
                },
                {
                  {j,i},
                  %{offset: offset_inv, rotation: rotation_inv} # inverse transformation
                },
              ]
            else
  ...
  def part_ii(filename \\ "lib/Q19/test") do
    ...
    offsets = compute_overlaps(measurements, 12)
    |> fill_transformations(n_scanners)
    |> Map.to_list()
    |> Enum.map(fn {{_, k}, %{offset: o}} -> {k, o} end)
    |> List.to_tuple()

    distances = 0..(n_scanners-3)
    |> Enum.flat_map(
      fn i ->
        i..(n_scanners-2)
        |> Enum.map(
