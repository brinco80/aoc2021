        ...
        case b do
          ...
          _ ->
            ...
            if row_bingo || col_bingo do
              ...
              # |> IO.inspect
              %{b | order: nb, bingo: true, bingo_number: new_number}
            else
              b
            end
        end
  ...
  def initialize_state(board_list) do
    boards =
      ...
      |> Enum.map(fn b ->
        status =
          b
          |> Enum.map(fn id -> {id |> Integer.to_string() |> String.to_atom(), false} end)
          |> Keyword.new()
  ...
  def part_i do
    ...
    case winner do
      ...
      {board, state} ->
        ...
        win =
          board[:status]
          |> Enum.reduce(0, fn {i, val}, acc ->
            if(!val, do: acc + (to_string(i) |> String.to_integer()), else: acc)
          end)
